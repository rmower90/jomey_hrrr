...
#   ./download_hrrr.sh YYYY MM OUT_DIR [ARCHIVE]
...
export HRRR_FC_HOURS=(1 2 3 4 5 6 7)
...
is_archive() {
  [[ "$1" == "${UofU_ARCHIVE}" || "$1" == "${AWS_ARCHIVE}" || "$1" == "${Google_ARCHIVE}" || "$1" == "${Azure_ARCHIVE}" ]]
}

# Dates list pattern (e.g., 20160201,20160202)
if [[ "$1" =~ ^[0-9]{8}(,[0-9]{8})*$ ]]; then
  # Mode: list of dates
  if [[ -z "$2" ]]; then
    echo "ERROR: OUT_DIR is required in dates mode."
    echo "Usage: $0 YYYYMMDD[,YYYYMMDD,...] OUT_DIR [ARCHIVE]"
    exit 1
  fi
  OUT_DIR="$2"
  IFS=',' read -r -a DATES <<< "$1"

  # Archive is optional 3rd arg
  if is_archive "$3"; then
    ARCHIVE="$3"
  else
...
# Year/Month pattern
elif [[ "$1" =~ ^[0-9]{4}$ && "$2" =~ ^[0-9]{1,2}$ ]]; then
  # Mode: year + month
  if [[ -z "$3" ]]; then
    echo "ERROR: OUT_DIR is required in year/month mode."
    echo "Usage: $0 YYYY MM OUT_DIR [ARCHIVE]"
    exit 1
  fi
  YEAR="$1"
  MONTH=$(printf "%02d" "$2")
  ...
  # Archive is optional 4th arg
  if is_archive "$4"; then
    ARCHIVE="$4"
  else
    ARCHIVE="${Google_ARCHIVE}"
  fi
  ...
  echo "Usage:"
  echo "  $0 YYYY MM OUT_DIR [ARCHIVE]"
  echo "  $0 YYYYMMDD[,YYYYMMDD,...] OUT_DIR [ARCHIVE]"
  exit 1
fi
